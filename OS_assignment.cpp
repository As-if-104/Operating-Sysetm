...
vector<Process> SRTFWithPriorityAging(vector<Process> processes, vector<TimeSlice> &gantt_chart)
{
    gantt_chart.clear();
    processes = prepareProcesses(processes);
    ...
    // Track waiting start time for aging
    vector<int> waiting_since(n);
    for (int i = 0; i < n; i++)
    {
        waiting_since[i] = processes[i].arrival_time;
    }

    const float aging_factor = 0.1;
    ...
    while (completed != n)
    {
        ...
        // Find all processes with minimum remaining time
        for (int i = 0; i < n; i++)
        {
            if (!is_completed[i] && processes[i].arrival_time <= current_time)
            {
                ...
                {
                    // Resolve tie using priority with aging
                    int waiting_time_i = current_time - waiting_since[i];
                    float effective_priority_i = processes[i].priority - (waiting_time_i * aging_factor);

                    int waiting_time_best = current_time - waiting_since[best_process_idx];
                    float effective_priority_best = processes[best_process_idx].priority - (waiting_time_best * aging_factor);
        ...
        if (best_process_idx == -1)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                if (!is_completed[i])
                {
                    next_arrival_time = min(next_arrival_time, processes[i].arrival_time);
                }
            }

            if (current_time < next_arrival_time && prev_process_id != 0)
            {
                gantt_chart.push_back({0, current_time, next_arrival_time}); // idle
                prev_process_id = 0;
            }
        ...
        if (prev_process_id != p.id)
        {
            gantt_chart.push_back({p.id, current_time, current_time + 1});
            prev_process_id = p.id;
        }
        else
        {
        ...
        p.remaining_time--;
        current_time++;
        waiting_since[best_process_idx] = current_time;
...
void compareAllAlgorithms(vector<Process> &original_processes, int time_quantum)
{
    ...
    result = SRTFWithPriorityAging(original_processes, gantt_chart);
    getAlgorithmMetrics(result, avg_response, avg_waiting, avg_turnaround);
    results.push_back({"SRTF with Priority base Aging", avg_response, avg_waiting, avg_turnaround});
...
int main()
{
    ...
    while (true)
    {
        ...
        cout << "7: SRTF with Priority based Aging (Novel Algorithm)" << endl;
        ...
        { // Adaptive Priority (Novel Algorithm)
            ...
            processes = SRTFWithPriorityAging(processes, gantt_chart);
            printGanttChart(gantt_chart);
            printProcessDetails(processes);
            calculateAverages(processes);
            break;
