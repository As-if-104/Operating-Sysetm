...
function admin_login {
    ...
    if grep -q "^$username|$password|" "$ADMIN_DB"; then
...
function admin_dashboard {
    ...
    echo -e "│  ${CYAN}3.${NC} View Results & Statistics       │"
    echo -e "│  ${CYAN}4.${NC} Change Password                 │"
    echo -e "│  ${CYAN}5.${NC} System Statistics              │"
...
function student_login {
    ...
    # Check credentials
    if grep -q "^$login_id|.*|$password$" "$STUDENT_DB" || grep -q "|$login_id|.*|$password$" "$STUDENT_DB" || grep -q ".*|$login_id|.*|$password$" "$STUDENT_DB"; then
        CURRENT_USER=$(grep -E "^$login_id|.*|$password$|.*|$login_id|.*|$password$|.*|$login_id|.*|$password$" "$STUDENT_DB" | cut -d'|' -f1)
        echo -e "${GREEN}Login successful!${NC}"
        loading_animation
        student_dashboard
    else
...
function take_exam {
    ...
    while IFS='|' read -r id name subject duration total_marks passing_marks num_questions; do
        echo -e "${WHITE}ID: $id - $name ($subject) - Duration: ${duration}min - Marks: $total_marks${NC}"
    done < "$EXAM_DB"
    
    echo -e "\n${CYAN}Enter Exam ID to take: ${NC}"
    read exam_id
...
function conduct_exam {
    ...
    while IFS='|' read -r question opt_a opt_b opt_c opt_d correct_answer marks; do
        show_header
        ...
        if [[ "${answer^^}" == "${correct_answer^^}" ]]; then
            score=$((score + marks))
            echo -e "${GREEN}Correct! (+$marks marks)${NC}"
        else
            echo -e "${RED}Wrong! Correct answer: $correct_answer${NC}"
        fi
        
        sleep 1
        ((question_num++))
    done < "$QUESTIONS_DIR/exam_$exam_id.txt"
    ...
    # Continuing from the exam result display...
...
function view_student_profile {
    ...
    if [[ -n "$student_info" ]]; then
        ...
        echo -e "${WHITE}📧 Email: ${CYAN}$email${NC}"
        echo -e "${WHITE}📱 Phone: ${CYAN}$phone${NC}"
        echo -e "${WHITE}⚧️  Gender: ${CYAN}$sex${NC}"
